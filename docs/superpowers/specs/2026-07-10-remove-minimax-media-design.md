...
Database initialization will create or update the schema first, then run parameterized cleanup statements for MiniMax image/video provider configuration rows. Repeated starts remain safe because deleting already-absent rows is a no-op.
